...
Going back to the `IRun` interface, if we think of the the Olympics then we have different types of runners, for example we could have a `Marathon Runner` and a `Sprinter`. Both can implement the the `IRun` interface, but in different ways.
